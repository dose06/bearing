...
if __name__ == "__main__":
    ...
    if full_df.empty:
        print("❌ full_df가 비어 있습니다."); exit()

    # Hold-out 처리: 각 Train 폴더에서 최소 RUL(고장 직전) 행만 떼어내고 삭제
    ...
    print(f"▶ 전체 시퀀스: {len(X_seq)}개")

    # 2차원으로 펼치기: (n_samples, WINDOW * n_features)
    n_features = X_seq.shape[2]
    X_flat = X_seq.reshape(X_seq.shape[0], WINDOW * n_features)

    # Stratified split (RUL 범주 사용)
    bins   = [-1, 30, 300, 2000, np.inf]
    labels = pd.cut(y_seq, bins=bins, labels=False)
    ...
    rf = RandomForestRegressor(
        n_estimators=100,       # 트리 200그루
        max_depth=12,           # 최대 깊이 10
        min_samples_leaf=5,     # 리프 당 최소 5샘플
        max_features="sqrt",    # 분할 시 전체 특성의 √개만 고려
        random_state=42,
        n_jobs=-1
    )
    ...
    # 모델 저장
    import joblib
    joblib.dump(rf, "rul_rf_all_sets.pkl")
    print("\n💾 랜덤 포레스트 모델이 'rul_rf_all_sets.pkl'로 저장되었습니다.")
